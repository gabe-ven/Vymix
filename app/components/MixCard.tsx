...
import { COLORS, getGradientByMood, getRandomGradient } from '../constants/colors';
...
export default function MixCard({ playlist, onPress }: MixCardProps) {
  ...
  const getGradientColors = () => {
    ...
    if (moodGradient === COLORS.musicGradients.electronic) {
      ...
      const selectedKey = gradientKeys[index] as keyof typeof COLORS.musicGradients;
      ...
      >
        <View className="flex-row items-center">
          ...
          <View className="flex-1">
            <Text className="text-white text-xl font-poppins-bold mb-3 leading-tight" numberOfLines={1}>
              {playlist.name}
            </Text>
            ...
            <View className="flex-row items-center justify-between">
              <View className="flex-row items-center">
                <View className="flex-row items-center bg-white/10 rounded-full px-3 py-1.5 mr-3">
                  <Ionicons name="musical-note" size={12} color={COLORS.primary.lime} style={{ marginRight: 4 }} />
                ...
                {playlist.createdAt && (
                  <View className="flex-row items-center bg-white/5 rounded-full px-3 py-1.5">
                    <Ionicons name="time-outline" size={12} color="rgba(255, 255, 255, 0.6)" style={{ marginRight: 4 }} />
