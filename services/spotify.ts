...
class SpotifyService {
  ...
  private getUserScopedKey(base: string, userId?: string | null): string {
    const uid =
      userId ?? this.currentUserId ?? auth().currentUser?.uid ?? 'anonymous';
    return `${base}_${uid}`;
  }
  ...
  private async loadTokens(): Promise<void> {
    try {
      ...
      const accessToken = await this.secureGetItem(
        this.getUserScopedKey('spotify_access_token')
      );
      const refreshToken = await this.secureGetItem(
        this.getUserScopedKey('spotify_refresh_token')
      );
      const tokenExpiry = await this.secureGetItem(
        this.getUserScopedKey('spotify_token_expiry')
      );
  ...
  public async loginToSpotify(userId?: string): Promise<string> {
    ...
    try {
      ...
      if (result.type === 'success' && result.url) {
        ...
        if (code) {
          const accessToken = await this.exchangeCodeForTokenPKCE(
            code,
            codeVerifier
          );
          ...
          if (userId) {
            console.log(
              'Marking user as connected after successful login:',
              userId
            );
  ...
  private async exchangeCodeForTokenPKCE(
    code: string,
    codeVerifier: string
  ): Promise<string> {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          `PKCE token exchange failed: ${response.status} ${response.statusText}`
        );
      }
  ...
  private async getValidAccessToken(): Promise<string> {
    ...
    if (!this.accessToken && this.refreshToken) {
      try {
        ...
        await this.clearTokens();
        throw new Error(
          'Access token expired and refresh failed. Please login again.'
        );
      }
    }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 429) {
          ...
          console.warn(
            `Spotify rate limit hit. Retry after ${Math.ceil(retryAfterMs / 1000)} seconds`
          );
  ...
  public async getRecommendations(params: {
    ...
    if (params.target_acousticness !== undefined) {
      queryParams.append(
        'target_acousticness',
        params.target_acousticness.toString()
      );
    }
    if (params.target_instrumentalness !== undefined) {
      queryParams.append(
        'target_instrumentalness',
        params.target_instrumentalness.toString()
      );
    }
    if (params.limit) {
  ...
  public async createPlaylist(
    ...
    const response = await this.makeRequest<CreatePlaylistResponse>(
  ...
  public async uploadPlaylistCoverImage(
    ...
    try {
      console.log(`Uploading cover image to playlist ${playlistId}`);
      ...
      if (originalSize > maxSizeBytes) {
        ...
        try {
          ...
          console.warn(
            'Failed to compress image, skipping cover upload:',
            error
          );
      ...
      if (finalSize > 200 * 1024) {
        console.warn(
          `Image still too large (${(finalSize / 1024).toFixed(2)}KB), skipping cover upload`
        );
        return;
      }
      ...
      console.log(
        `Successfully uploaded cover image to playlist ${playlistId}`
      );
    } catch (error) {
      console.error(
        `Failed to upload cover image to playlist ${playlistId}:`,
        error
      );
  ...
  private async uploadImageWithTimeout(
    playlistId: string,
    base64Data: string
  ): Promise<void> {
    const accessToken = await this.getValidAccessToken();
    ...
    try {
      console.log(
        `📤 Uploading ${((base64Data.length * 0.75) / 1024).toFixed(2)}KB image to Spotify...`
      );
  ...
  private async compressImageFromUrl(
    imageUrl: string,
    maxSizeBytes: number
  ): Promise<string> {
    try {
      ...
      for (const quality of qualityLevels) {
        try {
          ...
          console.log(
            `📏 Quality ${quality}: ${(estimatedSize / 1024).toFixed(2)}KB`
          );

          if (estimatedSize <= maxSizeBytes) {
            console.log(
              `Compressed with quality ${quality}, size: ${(estimatedSize / 1024).toFixed(2)}KB`
            );
      ...
      console.log(`Trying lowest quality with minimal size...`);
      ...
      console.log(
        `Using lowest quality, size: ${(estimatedSize / 1024).toFixed(2)}KB`
      );
      return finalBase64;
    } catch (error) {
